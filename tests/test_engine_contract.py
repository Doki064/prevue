...
def test_bad_then_good_sets_retried(
    ...
    def _run(_cmd, input=None, **_kwargs):
        ...
        #   stdin-only engine (claude-code-cli): prompt in `input` kwarg
        #   tempfile engine (cursor-cli): prompt written to the file at cmd[cmd.index("-f")+1]
        #     (checked before the generic "-p" check below: cursor-agent's base_argv
        #     already contains a literal "-p" flag of its own, unrelated to prompt delivery)
        #   size-gated argv-or-stdin engine (copilot-cli): small sample prompt fits under
        #     the argv ceiling, so it's captured from cmd[cmd.index("-p")+1]; `input` is None
        ...
        if input is not None:
            ...
            prompt_content = cmd_list[cmd_list.index("-p") + 1]
...
def test_vendor_argv(
    ...
    if engine_name == "copilot-cli":
        # Size-gated stdin-or-argv delivery (10-10 gap closure): the small sample
        # request's prompt fits under argv_prompt_max_bytes, so -p carries it and
        # nothing is piped to stdin — this reaches Copilot's OTEL-emitting code path.
        assert "-p" in cmd
        assert cmd[:3] == ["copilot", "-s", "--no-ask-user"]
