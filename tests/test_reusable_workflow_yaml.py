...
UPDATE_PRICING_WORKFLOW = (
    Path(__file__).resolve().parents[1] / ".github" / "workflows" / "update-pricing.yml"
)
INSTALL_SCRIPT = (
...
def test_update_pricing_jq_idempotency_empty_and_existing_pr() -> None:
    """WR-T04: cover update-pricing.yml's `gh pr list ... --jq` idempotency logic
    (create-PR vs edit-PR branch selection) — previously zero automated coverage.
    """
    if shutil.which("jq") is None:
        pytest.skip("jq not on PATH")

    text = UPDATE_PRICING_WORKFLOW.read_text(encoding="utf-8")
    match = re.search(r"gh pr list .*--jq '([^']+)'", text)
    assert match is not None, "gh pr list ... --jq '...' line not found in update-pricing.yml"
    ...
    assert empty_result.stdout.strip() == "", (
        f"Expected empty output for empty gh pr list result; got: {empty_result.stdout!r}"
    )

    existing_result = subprocess.run(
        ["jq", "-r", jq_filter],
        input=json.dumps([{"number": 42}]),
        capture_output=True,
        text=True,
        check=True,
    )
    assert existing_result.stdout.strip() == "42", (
        f"Expected '42' for existing gh pr list result; got: {existing_result.stdout!r}"
    )
