...
def test_load_config_warns_on_unknown_top_level_key(
    ...
    """WR-03: a typo'd top-level key (e.g. "revie") is surfaced, not silently dropped."""
    cfg_file = tmp_path / "prevue.yml"
    cfg_file.write_text("revie:\n  min_severity_to_fail: error\n")
    load_config(str(cfg_file))
    err = capsys.readouterr().err
    assert "revie" in err
    assert "unrecognized top-level" in err


def test_load_config_no_warning_for_known_top_level_keys(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """WR-03: recognized top-level keys never trigger the unknown-key warning."""
    cfg_file = tmp_path / "prevue.yml"
    cfg_file.write_text("review:\n  min_severity_to_fail: error\nengine:\n  name: cursor-cli\n")
    load_config(str(cfg_file))
    err = capsys.readouterr().err
    assert "unrecognized top-level" not in err
