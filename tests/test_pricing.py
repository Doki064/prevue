...
def test_null_override_falls_back_to_vendored_table() -> None:
    """WR-01: a null override row ({model: null}) means no override, not no cost."""
    _require_import()
    table = _sample_table()
    override = {"gpt-4o": None}
    usage = {"input": 1000, "output": 100, "cache_read": 0, "cache_creation": 0, "estimated": False}

    cost = compute_cost("openai", "gpt-4o", usage, override=override, table=table)  # type: ignore[misc]

    expected_table = 1000 * 2.5e-6 + 100 * 1e-5
    assert cost is not None, "Null override row must fall back to vendored table, not disable cost"
    assert abs(cost - expected_table) < 1e-12
