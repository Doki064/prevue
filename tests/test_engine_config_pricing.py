...
def test_pricing_rejects_string_scientific_notation_leaf() -> None:
    """YAML 1e-06 (no decimal point) parses as a string; must be rejected, not
    silently accepted and later crash compute_cost."""
    with pytest.raises(ValidationError, match="number or null"):
        EngineConfig.model_validate(
            {"name": "copilot-cli", "pricing": {"gpt-4o": {"input_cost_per_token": "1e-06"}}}
        )


def test_pricing_rejects_bool_leaf() -> None:
    """bool is an int subclass in Python but is never a valid cost value."""
    with pytest.raises(ValidationError, match="number or null"):
        EngineConfig.model_validate(
            {"name": "copilot-cli", "pricing": {"gpt-4o": {"input_cost_per_token": True}}}
        )
