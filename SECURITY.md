...
| 4 | Engine tool reach (PR metadata / network) | Adapters invoke CLIs with **no `--allow-tool` flags** (code-audited). Copilot: `copilot -s --no-ask-user`. Claude: `claude -p --output-format json`. Cursor: `cursor-agent -p --output-format json -f`. Antigravity (`agy -p`) is registered but `functional=False` — `require_functional_adapter` rejects it before any subprocess runs; when it does run (future), invocation goes through a `script -qec` pseudo-TTY wrapper (see `cli_adapter.py`). **Live engine tool-posture verify (D-08) is a required pre-production checkpoint** — run each adapter in a sandbox PR and confirm no unexpected tool calls occur before enabling in merge-gate workflows. See the step-by-step checklist in [docs/consumer-setup.md](docs/consumer-setup.md#engine-tool-posture-check-before-merge-gates-d-08) and the 07-05 UAT checklist. |

**T-08 note (`--bare` removal, 10-THERMOS):** `claude-code-cli` omits `--bare` because `--bare` strictly requires `ANTHROPIC_API_KEY`/`apiKeyHelper` auth and never reads OAuth/keychain, which would break `CLAUDE_CODE_OAUTH_TOKEN`-based CI auth. Trialed `--safe-mode` as a way to disable CLAUDE.md/skills/plugin/hook/MCP auto-discovery (the same things `--bare` skips) without narrowing the auth surface, on the theory that auto-discovery was causing CI timeouts on headless Actions runners. A live CI run disproved this: `engine_meta.duration_s` showed the time was genuine Claude API generation time for a large diff (56k review tokens), not startup overhead — `--safe-mode` measured no improvement and was reverted. Fix instead: `ReviewRequest.budget_seconds` raised 300s → 600s (`models.py`). Not yet consumer-configurable; re-evaluate if large-diff timeouts recur. Re-evaluate `--bare` itself if Claude Code CLI ever adds OAuth + bare compat.
