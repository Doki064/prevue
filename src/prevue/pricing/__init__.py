...
def _lookup_row(
    ...
    # D-06c: engine.pricing override takes precedence — but a null override
    # row (`{model: null}`) means "no override, use the vendored default"
    # (WR-01), so only short-circuit when the row is actually present.
    if override:
        if model in override and override[model] is not None:
            return override[model]
        if norm in override and override[norm] is not None:
