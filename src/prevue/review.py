...
from prevue import review_classify
...
class CheckRunPublishFailure(RuntimeError):
    """Raised when conclude_review_check fails after machine output was already emitted.

    Distinct from a bare RuntimeError so cli.py can skip its generic hard-fail
    output emission — re-emitting here would overwrite the real findings/tokens
    already written with a minimal error summary.
    """
...
def run_review(
    *,
    ...
    # require_functional_adapter stays here — tests patch prevue.review.require_functional_adapter.
    try:
    ...
    # Review model: PREVUE_MODEL > COPILOT_MODEL > models.review > engine.model
    ...
    if not check_published:
        # CheckRunPublishFailure (not a bare RuntimeError): machine output above already
        # carries the real findings/tokens — cli.py must not let its generic hard-fail
        # handler clobber that with a minimal error summary (T-04 / phase-10 review).
        raise CheckRunPublishFailure("Failed to publish review check run")
