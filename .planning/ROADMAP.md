...
| 10. Boundary Contracts | 11/11 | Complete   | 2026-07-06 |
...
**Plans**: 11 plans (4 waves + gap closure)
Plans:
**Wave 1**
...
- [x] 10-09-PLAN.md — Gap closure: rewrote usage.py::_parse_copilot_otel for the real flat span-per-line JSONL schema (10-08's "no CI-viable mechanism" conclusion was stale); usage_capture flipped back to "otel-jsonl"; COPILOT_OTEL_FILE_EXPORTER_PATH wired into both reusable workflows
- [x] 10-10-PLAN.md — Gap closure (recurrence): Copilot CLI 1.0.67 only emits OTEL when invoked with -p; prevue's stdin-delivery invocation shape never triggered it. Adds a size-gated stdin-or-argv prompt-delivery mode (64 KiB ceiling) so small/medium PRs reach the OTEL-emitting -p path while large PRs keep the Phase-1-proven stdin fallback (no ARG_MAX regression)
- [x] 10-11-PLAN.md — Gap closure (THIRD recurrence): review()'s tempfile.mkdtemp() pre-created the per-call OTEL path as an empty directory, silently defeating Copilot's file exporter (it only writes when the exact path doesn't already exist). Replaces it with a non-filesystem-touching unique path (os.path.join + uuid4 hex), preserving WR-01 per-call isolation; regression test guards against a fourth recurrence
