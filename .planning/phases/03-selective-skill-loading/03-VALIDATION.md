...
| SKIL-02/D-11 | lean content floor: baseline ≥4 bullets (all); opener + backtick examples + severity footer for THIN_SKILL_KEYS | unit | `uv run pytest tests/test_skills_builtin.py::test_builtin_skill_content_lean_floor -x` | tests/test_skills_builtin.py | ✅ green |
...
|----------|-------------|------------|-------------------|
| Built-in skill content quality — full depth parity across all 11 skills + token-budget content packing | SKIL-02 | Phase 6 scope (OUTP-04); content judgment not fully assertable | Read each `src/prevue/skills/<bundle>/*.md`; confirm genuine review guidance, not placeholders. Full depth parity → Phase 6 (OUTP-04). |

> **Note:** Two-tier lean-floor rubric is now automated (see Per-Task Verification Map). Baseline ≥4 bullets applies to all 11 skills; strict committed-secrets bar (opener + backtick examples + severity footer) applies to `THIN_SKILL_KEYS` only (`frontend/component-state.md`, `infra/ci-workflow-hardening.md`).
...
**Approval:** approved 2026-06-12 — all automated requirements covered; 114 tests green
...
## Validation Audit 2026-06-12 (re-run)
...
| Resolved | 0 |
| Escalated | 0 |
| Tests verified | 13 mapped / 114 total — all green |
