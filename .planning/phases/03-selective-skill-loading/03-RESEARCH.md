...
1. **`applies-to` schema purity vs Agent Skills spec.** — **RESOLVED (D-02 locked)**
   - **Decision:** Keep top-level `applies-to` per D-02. Prevue-specific routing field; Agent Skills portability preserved via `Skill.model_config = ConfigDict(extra="ignore")` so files with a `metadata:` map still parse. Mechanical migration to `metadata.applies-to` deferred unless strict spec-compliance becomes a requirement.

2. **Cross-cutting glob breadth (e.g. `input-validation.md: **/*.py`).** — **RESOLVED (security-as-floor)**
   - **Decision:** Keep broad security globs. Security skills (input-validation, committed-secrets) load on most backend PRs by design — security-first posture. Backend-only success criterion holds: frontend/data/infra skills don't match `.py` paths. Token-budget tightening deferred to Phase 6 (OUTP-04).

3. **Shared `canonical_index()` helper.** — **RESOLVED (lifted in Plan 03-02)**
   - **Decision:** Single `canonical_index(name)` in `classify/models.py`; imported by `router.py`, `comments.py`, and `skills/loader.py`. Verified shipped: `src/prevue/classify/models.py:18`, consumed by all three modules.
